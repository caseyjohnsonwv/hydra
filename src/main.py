...
total_time = 0
edit_list = []
while len(clips) > 0 and ((ARGS.max_length is not None and total_time < int(ARGS.max_length)) or (ARGS.max_clips is not None and len(edit_list) < int(ARGS.max_clips))):
    # choose clip
    if ARGS.randomize:
        clip = choice(clips)
    else:
...
output = mpe.concatenate_videoclips(final_clips, method='compose')
